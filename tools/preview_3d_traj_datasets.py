...
from core.kinematics import spatial_fk
...
def _project_root() -> str:
    # tools/preview_3d_traj_datasets.py -> repo root is ../
...
def _resolve_outdir(outdir: str) -> str:
    p = str(outdir).strip() or "tools/results/traj_preview_3d"
    if os.path.isabs(p):
        return p
    return os.path.join(_project_root(), p)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--outdir", default="tools/results/traj_preview_3d", help="Output directory")
