...
def main() -> None:
    ...
    parser.add_argument("--dataset", default="2d_ellipse")
    ...
    metrics = evaluate_projection_metrics(
        ...
        x_train=x_train,
        cfg=eval_cfg,
        project_fn=lambda m, x0, eps: (
            project_trajectory(
                m, torch.from_numpy(x0.astype(np.float32)).to(cfg.device), cfg, f_abs_stop=eps
            )[0][-1],
            np.full((len(x0),), float(cfg.proj_steps), dtype=np.float32),
        ),
    ...
    tau = eval_cfg.eval_tau_ratio * float(np.mean(grid.max(axis=0) - grid.min(axis=0)))
    true_on = d_eval < tau
