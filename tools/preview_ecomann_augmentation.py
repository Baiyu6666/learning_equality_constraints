...
from methods.baseline_udf.baseline_udf import knn_normal_bases
...
def _resolve_outdir(outdir: str) -> str:
    p = str(outdir).strip() or "tools/results/ecomann_aug_preview"
    if os.path.isabs(p):
        return p
...
def main() -> None:
    ...
    ap.add_argument("--outdir", default="tools/results/ecomann_aug_preview", type=str)
