...
def _plot_dataset_bar(
    ...
    # Style aligned to outputs_unified/analysis/benchmark/plot_benchmark_boxplots_paper.py
...
def _plot_paper_single_panel_line(
    *,
    ...
    target_methods = ["eikonal", "ecomann"]
    if len(scales) == 0:
        return
    ...
    method_style = {"eikonal": ("ONCL (Ours)", "-"), "ecomann": ("ECoMaNN", "--")}
...
def main() -> None:
    ...
    parser.add_argument(
        "--methods",
        nargs="+",
        default=["eikonal", "ecomann"],
        help="Method order in grouped bars.",
    )
    parser.add_argument(
        "--outdir",
        default="outputs_unified/bench/scale_impact_analysis",
