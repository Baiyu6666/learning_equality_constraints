...
_LEGACY_SCRIPT = os.path.join(_PROJECT_ROOT, "outputs_unified", "bench", "all_2d3d6d_5seed", "analyze_dataset_corr_cov_proj.py")
...
def _resolve_bench_dir(bench: str) -> str:
    ...
    if os.path.isabs(b):
        return b
    p1 = os.path.join(_PROJECT_ROOT, "outputs_unified", "bench", b)
    if os.path.isdir(p1):
        return p1
...
def main() -> None:
    ...
    p.add_argument("--bench", required=True, help="Benchmark name (under outputs_unified/bench) or absolute path")
