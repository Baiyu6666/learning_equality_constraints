...
import plot_benchmark_boxplots as base
...
METHOD_PRIORITY = ["eikonal", "delta", "eikonal_single", "vae", "ecomann"]
...
def _plot_codim_gt1_eikonal_compare(
    *,
    out_path: str,
    ylabel: str,
    rows: list[dict[str, object]],
    ...
    methods = ["eikonal", "eikonal_single"]
    ...
    width = 0.34
    colors = {"eikonal": "#4C78A8", "eikonal_single": "#E45756"}
...
def main() -> None:
    ...
    p.add_argument("--methods", type=str, default="delta,eikonal,vae,ecomann", help="Comma-separated methods. Default: delta,eikonal,vae,ecomann")
    ...
    p.add_argument("--metrics", type=str, default="proj_manifold_dist,pred_precision,gt_to_learned", help="Comma-separated metrics to plot.")
    ...
    p.add_argument("--traj-overlay-methods", type=str, default="eikonal,ecomann", help="Comma-separated methods for traj/non-traj overlay plot only. Default: use --methods.")
    p.add_argument("--codim-gt1-compare", default=True, help="Also plot codim>1 dataset comparison between eikonal and eikonal_single.")
    args = p.parse_args()
    ...
    for metric in metrics:
        ...
        if bool(args.codim_gt1_compare):
            ...
            _plot_codim_gt1_eikonal_compare(
                out_path=out_codim,
                ylabel=metric_label,
                rows=rows,
                datasets=datasets,
