...
from levelset_energy_algorithm import (
    Config,
    compute_eval_eps_used,
    evaluate_metrics,
    generate_dataset,
    knn_normals_with_quality,
...
def run_one(
    ...
    for idx, name in enumerate(datasets):
        ...
        eval_rng = np.random.default_rng(eval_seed + idx * 1000)
        x_eval = sample_eval_points(x_train, grid, cfg, eval_rng)

        if method == "energy":
            model, _, _, _, _, _ = train_with_data(cfg, x_train)
        elif method in ("margin", "delta"):
            knn_norm_estimation_points = max(
                cfg.knn_norm_estimation_min_points,
                int(round(cfg.knn_norm_estimation_ratio * len(x_train))),
            )
            n_hat, _, _ = knn_normals_with_quality(x_train, knn_norm_estimation_points, cfg)
    ...
    with wandb.init() as run:
        ...
        for idx, name in enumerate(datasets):
            ...
            if method == "energy":
                ...
                n_hat, _, _ = knn_normals_with_quality(
...
def main() -> None:
    ...
    if wb_enable:
        ...
        for method, spec in methods_cfg.items():
            ...
            for p in grid:
                run = run_one(cfg_base, method, p, datasets, cfg_base.eval_seed)
                all_runs.append(run)
