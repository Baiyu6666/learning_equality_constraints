...
class ECMNNDatasetLoader(DatasetLoader):
    ...
    def __init__(self, dataset_filepath, *args, **kwargs):
        ...
        N_local_neighborhood_float = N_local_neighborhood_mult * 2 * (2 ** dim_ambient)
        N_local_neighborhood = max(2, int(round(N_local_neighborhood_float)))
        ...
        mode_ret = stats.mode(argmax_diff_eigval, axis=None, keepdims=False)
        if hasattr(mode_ret, "mode"):
            mode_val = mode_ret.mode
        elif isinstance(mode_ret, (tuple, list)) and len(mode_ret) > 0:
            mode_val = mode_ret[0]
        else:
            mode_val = mode_ret
        dim_tangent_space = int(np.asarray(mode_val).reshape(-1)[0]) + 1
        dim_normal_space = dim_ambient - dim_tangent_space
        inferred_dim_tangent_space = int(dim_tangent_space)
        inferred_dim_normal_space = int(dim_normal_space)
        if dim_normal_space_override is not None:
            forced_dim_normal_space = int(dim_normal_space_override)
            assert(forced_dim_normal_space >= 1)
            assert(forced_dim_normal_space < dim_ambient)
            if forced_dim_normal_space != inferred_dim_normal_space:
                print("[warn] [ecomann-codim] LPCA estimated normal dim = %d, "
                      "but forcing normal dim = %d (gt codim)." %
                      (inferred_dim_normal_space, forced_dim_normal_space))
            dim_normal_space = forced_dim_normal_space
            dim_tangent_space = dim_ambient - dim_normal_space
        ...
        if is_performing_data_augmentation:
            ...
            for i in range(1, N_normal_space_traversal+1):
                ...
                for level_mult_eigvec in level_mult_eigvec_list:
                    ...
                    N_aug = len(valid_idx)
                    accepted_counts.append(int(N_aug))
                    if N_aug == 0:
                        continue
                    ...
                    if is_optimizing_signed_siamese_pairs or is_computing_all_cost_components:
                        ...
                        valid_pair_counts.append((int(N_valid_siam_reflection_pairs), int(len(valid_idx))))
                ...
                if len(accepted_counts) > 0:
                    acc = np.array(accepted_counts, dtype=np.float64)
                    if len(valid_pair_counts) > 0:
                        vp = np.array([v for v, _ in valid_pair_counts], dtype=np.float64)
                        vd = np.array([d for _, d in valid_pair_counts], dtype=np.float64)
                        ratio = np.divide(vp, np.maximum(vd, 1.0))
                        print(
                            '  Aug summary: accepted(mean/min/max)=%.1f/%d/%d of %d | '
                            'valid_pairs(mean/min/max)=%.1f/%d/%d (ratio mean=%.3f)' % (
                                float(np.mean(acc)),
                                int(np.min(acc)),
                                int(np.max(acc)),
                                int(N_data),
        ...
        if is_performing_data_augmentation:
            aug_dir = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "..", "data", "augmented")
            )
            create_dir_if_not_exist(aug_dir)
            for key, value in dataset_dict.items():
                np.save(os.path.join(aug_dir, key), value)
