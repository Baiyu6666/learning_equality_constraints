...
class DatasetLoader(object):
    ...
    def get_train_valid_test_dataloaders(self, train_fraction=0.8, valid_fraction=0.1, batch_size=128):
        ...
        # Use single-process loading for broad compatibility (e.g., restricted environments).
        batch_train_loader = DataLoader(self.dataset, batch_size=batch_size, sampler=train_sampler, num_workers=0)
        batch_valid_loader = DataLoader(self.dataset, batch_size=batch_size, sampler=valid_sampler, num_workers=0)
        batch_test_loader = DataLoader(self.dataset, batch_size=batch_size, sampler=test_sampler, num_workers=0)
