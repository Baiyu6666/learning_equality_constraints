...
def align_normal_space_eigvecs(dataset_dict, N_local_neighborhood_mst=2,
    ...
    if N_data <= 1:
        # Nothing to align for degenerate inputs.
        return dataset_dict['cov_nullspace']
    max_neighbors = max(1, N_data - 1)
    kd_tree = cKDTree(data=dataset_dict['data'])
    is_dag_extraction_successful = False
    dag_search_start_time = time.perf_counter()
    dag_search_iters = 0
    dag_k_start = int(N_local_neighborhood_mst)
    while not is_dag_extraction_successful:
        dag_search_iters += 1
        k_eff = int(min(max(1, int(N_local_neighborhood_mst)), max_neighbors))
        start_time = time.perf_counter()
        sparse_matrix_index_I = list()
        sparse_matrix_index_J = list()
        sparse_matrix_value_V = list()
        for i in range(N_data):
            # compute nearest neighbors of size k_eff (exclude self):
            [dists, indices] = kd_tree.query(dataset_dict['data'][i], k=k_eff + 1)
            indices = indices[1:]
            dists = dists[1:]
            for j, dist in zip(indices, dists):
                # cKDTree may emit sentinel index == N_data when k exceeds valid neighbors.
                if j < 0 or j >= N_data:
                    continue
                dist = float(dist)
                if not np.isfinite(dist):
                    continue
                # Keep edge weights strictly positive; zero may be treated as "no edge"
                # by downstream sparse-graph routines.
                if dist <= 0.0:
                    dist = 1e-12
        ...
        dag_build_sec = float(time.perf_counter() - start_time)
        ...
        if N_components == 1:
            ...
            if k_eff >= max_neighbors:
                raise RuntimeError(
                    'IOSA graph disconnected even at k=N-1. '
                    'Likely invalid/degenerate data graph (duplicates/NaNs/disjoint components).'
                )
            N_local_neighborhood_mst += 1
    print(
        'IOSA graph connectivity: '
        'k_start=%d, k_final=%d, attempts=%d, total=%.3fs '
        '(last: sparse=%.3fs, mst=%.3fs, dag=%.3fs)' % (
            dag_k_start,
            k_eff,
            dag_search_iters,
            float(time.perf_counter() - dag_search_start_time),
            sparse_build_sec,
            mst_build_sec,
            dag_build_sec,
        )
    ...
    for si in range(2):  # si: source index (either the 1st or 2nd copy of the eigenvector set)
        for di in range(2):  # di: destination index (either the 1st or 2nd copy of the eigenvector set)
            ...
            if not terminated_early:
                print('Source Idx %d, Dest Idx %d, Epoch %2d: ' % (si, di, N_epoch - 1))
                print('           mean_loss = ', current_mean_loss)
                print('           loss[:10] = ', current_losses[:10])
                if len(window_past_mean_losses_list) >= 1:
                    np_window_past_mean_losses = np.array(window_past_mean_losses_list)
                    print('           mean_past_mean_losses = ', np.mean(np_window_past_mean_losses))
                    print('           std_past_mean_losses = ', np.std(np_window_past_mean_losses))
    ...
    if hasattr(child2parent_orig_losses, "numpy"):
    ...
    if hasattr(child2parent_osa_opt_losses, "numpy"):
...
if __name__ == "__main__":
    ...
    if hasattr(node2root_osa_losses, "numpy"):
