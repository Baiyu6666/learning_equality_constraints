...
def _normalize_ee_link_index(v: object) -> int | None:
    if v is None:
        return None
    iv = int(v)
...
def get_ur5_backend(
    ...
    try:
        ...
        _UR5_BACKEND = UR5PyBulletKinematics.from_settings(
            ...
            ee_link_index=_normalize_ee_link_index(kin.get("ee_link_index")),
...
def ur5_fk_chain_n6(q_batch: np.ndarray, use_pybullet: bool = True) -> np.ndarray:
    q_batch = np.asarray(q_batch, dtype=np.float32)
    backend = get_ur5_backend() if use_pybullet else None
    if backend is not None:
        chain, _, _ = backend.fk_batch(q_batch)
        return chain.astype(np.float32)

    # Analytic fallback: axes z, y, y, y, x, z; link offsets along local +x.
    t = q_batch.shape[0]
    l = [float(v) for v in UR5_LINK_LENGTHS]
    out = np.zeros((t, 7, 3), dtype=np.float32)
    for i in range(t):
        qi = q_batch[i]
...
def ur5_tool_axis_n6(q_batch: np.ndarray, use_pybullet: bool = True) -> np.ndarray:
    q_batch = np.asarray(q_batch, dtype=np.float32)
    backend = get_ur5_backend() if use_pybullet else None
    if backend is not None:
        axis = backend.tool_axis_batch(q_batch)
